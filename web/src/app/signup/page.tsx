          ...
          <div className="max-w-md mx-auto">
            <div className="text-center mb-8">
              ...
              <p className="text-white/60">
                Join ALIRA and start your transformation.
              </p>

              {(program || membership) && (
                <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#d4af37]/10 border border-[#d4af37]/20 text-[#d4af37] text-sm">
                  <Check className="w-4 h-4" />
                  {program && `Selected: ${program.charAt(0).toUpperCase() + program.slice(1)} Program`}
                  {membership && `Selected: ${membership.charAt(0).toUpperCase() + membership.slice(1)} Membership`}
                </div>
              )}
            </div>
            ...
            <div className="p-8 rounded-3xl bg-white/5 border border-white/10">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <label className="flex items-start gap-4 text-white/60 text-sm">
                  <input 
                    type="checkbox" 
                    checked={formData.agreeToTerms}
                    onChange={(e) => setFormData({...formData, agreeToTerms: e.target.checked})}
                    className="w-4 h-4 mt-0.5 rounded border-white/20 bg-white/5 text-[#8b5cf6]" 
                  />
                  <span>
                    I agree to the{" "}
                    <Link href="/terms" className="text-[#8b5cf6] hover:text-[#a78bfa]">Terms of Service</Link>
                    {" "}and{" "}
                    <Link href="/privacy" className="text-[#8b5cf6] hover:text-[#a78bfa]">Privacy Policy</Link>
                  </span>
                </label>
                ...
                <button 
                  ...
                  className="w-full py-4 bg-gradient-to-r from-[#8b5cf6] to-[#a78bfa] text-white font-bold tracking-[0.1em] uppercase rounded-full hover:shadow-[0_10px_30px_rgba(139,92,246,0.4)] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div className="mt-6 text-center">
                <p className="text-white/60">
                  ...
                  <Link href="/login" className="text-[#8b5cf6] hover:text-[#a78bfa] font-semibold">
                    Sign in
                  </Link>
                </p>
              </div>
